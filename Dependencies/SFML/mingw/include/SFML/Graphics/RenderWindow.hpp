...
public:
    ...
    ///
    /// \deprecated
    /// Use a sf::Texture and its sf::Texture::handleInput(const Window&)
    /// function and copy its contents into an sf::Image instead.
    /// \code
    /// sf::Vector2u windowSize = window.getSize();
    /// sf::Texture texture;
    /// texture.create(windowSize.x, windowSize.y);
    /// texture.handleInput(window);
    /// sf::Image screenshot = texture.copyToImage();
    /// \endcode
    ///
    /// This is a slow operation, whose main purpose is to make
    /// screenshots of the application. If you want to handleInput an
    /// image with the contents of the window and then use it for
    /// drawing, you should rather use a sf::Texture and its
    /// handleInput(Window&) function.
